...
HAL_AFEStatusTypeDef HAL_AFEReadWriteRegister(AFE_HandleTypeDef *hafe,
	...
	// Configure SPI
	MX_SPI_AFE_OP_Config(hafe->SPI_handle);
	...
	HAL_SPI_TransmitReceive(hafe->SPI_handle, dataW, dataR, dataSize, Timeout);
	while (HAL_SPI_GetState(hafe->SPI_handle) != HAL_SPI_STATE_READY) {
		if ((HAL_GetTick() - tickstart) >= Timeout) {
			errorcode = HAL_AFE_TIMEOUT;
			return errorcode;
		}
	}
...
HAL_AFEStatusTypeDef HAL_AFEWriteRegister(AFE_HandleTypeDef *hafe,
	...
	HAL_AFEStatusTypeDef errorcode = HAL_OK;
